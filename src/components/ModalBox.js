...
const ModalBox = React.createClass({
  ...
  createPanResponder() {
    ...
    const onPanRelease = (evt, state) => {
      ...
      inSwipeArea = false;
      if (this.props.entry === 'top' ? -state.dy > this.props.swipeThreshold
        : state.dy > this.props.swipeThreshold) { this.animateClose(); } else { this.animateOpen(); }
    };
    ...
    const onPanMove = (evt, state) => {
      ...
      state.customY = state.dy + this.state.positionDest; // eslint-disable-line

      animEvt(evt, state);
    };

    const onPanStart = (evt) => {
      if (!this.props.swipeToClose || this.props.isDisabled || (this.props.swipeArea
